...
compare_plot <- function(
  ...
  p <- p +
    scale_fill_manual(values = final_colors, name = fill_var) +
...
#' 添加主图层
#' @keywords internal
add_main_geom <- function(plot_type, pos, is_grouped, fill_var,
                          violin_alpha = 0.8, violin_linewidth = 0.5) {
  geom_list <- list()
...
add_jitter_points <- function(
  ...
  # 处理填充颜色
  if (point_fill == "same") {
    # 使用与主图相同的颜色，但通过 alpha 控制透明度
    # 此时 fill 由 aes 继承，所以不设置固定值
    use_fill_aes <- TRUE
    fill_value <- NULL
  } else {
    # 使用指定的固定颜色
