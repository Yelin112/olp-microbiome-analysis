...
.norm_rarefy <- function(abund_table, sample.size, rngseed, replace) {
  ...
  if (sum(keep) == 0) {
    stop("所有样本文库深度均小于 sample.size = ", sample.size)
  }
  if (sum(!keep) > 0) {
    message(
      "Rarefy: 移除 ",
...
.norm_gmpr <- function(abund_table, intersect.no, ct.min) {
  ...
  for (i in seq_len(n_samples)) {
    ...
    for (j in seq_len(n_samples)) {
      ...
      if (sum(both_nonzero) < intersect.no) {
        next
      }
  ...
  if (any(is.na(size_factors))) {
    warning(
      "GMPR: ",
      sum(is.na(size_factors)),
      " 个样本无法计算 size factor, 使用 1 替代"
    )
...
plot_norm_comparison <- function(raw, normalized, method_name = "") {
  ...
  ggplot2::ggplot(
    lib_df,
    ggplot2::aes(x = reorder(Sample, LibSize), y = LibSize, fill = Type)
  ) +
