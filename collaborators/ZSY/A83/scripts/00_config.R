...
# 项目：A83（重测批次）CAF/NF 条件培养基处理 + shART3 敲低救援效应
# 分组：Ctrl / NF / CAF / shART3（各3重复，单一细胞系批次，无需按细胞系拆分）
#
# 与 collaborators/ZSY/LM_83 项目为同一研究思路的重测批次，脚本结构照搬，
# 但本批次原始表达量为 FPKM（非 TPM），注释列也不同（coding_type 而非
# gene_biotype），且 counts/FPKM 各自已是单一注释矩阵，无需 AccID 去重合并。
# ================================================================
...
PROJ_DIR <- file.path(PROJECT_ROOT, "collaborators/ZSY/A83")
...
# 单一细胞系批次；沿用 LM_83 项目里"按细胞系循环"的脚本结构，
# 此处 CELL_LINES 只有一个元素，循环体自然只跑一轮
CELL_LINES <- c("A83")

# 有序分组：Ctrl(基线) -> NF(正常成纤维细胞上清) -> CAF(癌相关成纤维细胞上清)
#           -> shART3(敲低ART3后的CAF处理，预期部分救援回Ctrl/NF)
GROUP_SETS <- list(
  full = c("Ctrl", "NF", "CAF", "shART3"),
  no_NF = c("Ctrl", "CAF", "shART3")
)

# 当前激活的分组变体；后续如需去掉 NF 重新分析救援效应，
# 只需改为 "no_NF" 并重跑 02→04 脚本，01 的清洗结果无需重新生成
ACTIVE_VARIANT <- "no_NF"
...
P <- list(
  ...
  expr_cutoff = 1, # 聚类前的最低表达量过滤（任一组均值 FPKM，可调）
  # LRT 显著性阈值，决定进入聚类的基因全集；先用标准0.05，
  # 若02阶段信号较弱（参考LM_83项目"83"细胞系的经验）可放宽至0.10
  lrt_padj_cutoff = list("A83" = 0.05),
  min_std = 0.1, # clusterData() 内部低方差基因过滤阈值
  enrich_topn = 12, # 每个簇富集展示的 top term 数
  # elbow 图确定后手动填入对应方法的簇数，未填(NA)时聚类脚本会报错，
  # 避免在未经人工确认 K 值的情况下静默产出结果
  k_clusters = list(
    "A83" = list(kmeans = 5, mfuzz = 5)
  )
)
...
# 这里放的 p.adjust 值）。（与 LM_83 项目 00_config.R 中的同名函数保持一致）
